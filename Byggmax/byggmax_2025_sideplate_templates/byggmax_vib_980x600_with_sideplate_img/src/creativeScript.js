...
window.addEventListener('lemonpi.content/ready', event => {
  ...
  if (useAnimation === 'no') {
    ...
    // Define the badge pulsating animation function with a "heartbeat" effect
    function animateBadge() {
    gsap.timeline()
      .to(badge, { scale: 1.2, duration: 0.2, ease: "power1.inOut" }) // First bump
      .to(badge, { scale: 1, duration: 0.2, ease: "power1.inOut" })   // Return to normal size
      .to(badge, { scale: 1.15, duration: 0.2, ease: "power1.inOut" }) // Second bump
      .to(badge, { scale: 1, duration: 0.2, ease: "power1.inOut" });  // Return to normal size
    }
