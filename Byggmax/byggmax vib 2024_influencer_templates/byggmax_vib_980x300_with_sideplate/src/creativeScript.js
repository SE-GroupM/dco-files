...
window.addEventListener('lemonpi.content/ready', event => {
 ...
 var leftTejpPosition = parseInt(local_content.tejp_position_left.value);

 $('#sideplate').css({
  left: leftTejpPosition + 'px',
});
...
// Deklarera och tilldela alla nödvändiga variabler i början av if-satsen
var videoAdjustmentTop = parseInt(local_content.video_placement_topPosition.value);
var videoAdjustmentLeft = parseInt(local_content.video_placement_leftPosition.value);
var videoWidth_adjustment = parseInt(local_content.videoPlayer_width.value);
var playerBgColor = '#FFe632'; // Denna är konstant, kan deklareras här

// Hämta videoelementet en gång
const videoElement = document.querySelector('.player'); // Antar att det är en klass '.player' baserat på jQuery-selektorn

// Huvudvillkoret för att köra justeringarna
// Kontrollerar om topPosition har ett giltigt numeriskt värde (inte tom sträng och inte NaN)
if (!isNaN(videoAdjustmentTop) && local_content.video_placement_topPosition.value !== '') {

// Lägg till 'px' direkt om det är ett giltigt nummer
// Om videoAdjustmentTop redan är NaN (från parseInt av en ogiltig sträng), kommer den inte att användas
// Vi vill bara lägga till 'px' om det är ett nummer, annars kan vi få "NaNpx"
let topPositionValue = videoAdjustmentTop + 'px';
let leftPositionValue = videoAdjustmentLeft + 'px'; // Antar att leftPosition också ska ha 'px'

// Använd videoElement om det hittades, annars faller vi tillbaka på jQuery-selektorn
// Det är mer effektivt att använda den redan hämtade referensen
if (videoElement) {
    videoElement.style.position = 'absolute';
    videoElement.style.top = topPositionValue;
    videoElement.style.left = leftPositionValue;
    videoElement.style.backgroundColor = playerBgColor;
} else {
    // Fallback till jQuery om querySelector inte hittade elementet
    $('.player').css({
        'position': 'absolute',
  ...
  // Justera bredden om videoWidth_adjustment är ett giltigt nummer
  if (!isNaN(videoWidth_adjustment)) {
      if (videoElement) {
          videoElement.style.width = videoWidth_adjustment + 'px';
      } else {
          $('.player').css({
              width: videoWidth_adjustment + 'px',
          });
      }
  }
}
