...
console.clear();
  ...
  var leftDirectionVar = 930; // Controlls the slider value, on how much it should move the images.
  ...
  for (let i = 1; i < 4; i++) {
    if (i == 3) {
      if(use_slide_3_text) {
        ...
        'font-family': 'CEWE',
        'font-size':'25px',
        'left': 20 + (i-1)* (leftDirectionVar) + 'px',
        'color':'white',
        'text-align':'center',
        'width':'400px',
        'top': '40px'
  ...
  if (goAhead) {
    ...
    function goBackTostart (currentIndex, direction) {
      if (currentIndex === 3 && direction =='-='){
        ...
        tl.to(".product_image", 0.7, { left: direction + (-930*2), ease: Power1.easeInOut, delay: 2.2}, 0)
      }else{
...
// if (parseInt(slider_element_1_left_value) > 10 && parseInt(slider_element_1_left_value) < 300) {
//   currentURL = url_1;
// }
// // URL 2 - if slider_element_1_left_value is greater than -300 and less than -200
//   else if (parseInt(slider_element_1_left_value) > -1000 && parseInt(slider_element_1_left_value) < -600) {
//     currentURL = url_2;
//   } 
//  // URL 3 - if slider_element_1_left_value is greater than -650 and less than -500
//  else if (parseInt(slider_element_1_left_value) > -2000 && parseInt(slider_element_1_left_value) < -1200) {
//   currentURL = url_3;
//   }
//   // Open the URL
