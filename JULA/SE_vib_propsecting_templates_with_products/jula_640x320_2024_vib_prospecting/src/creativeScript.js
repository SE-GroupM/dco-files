  ...
  if (ccVideo != '') {
    ...
    function updateProductDisplay(index) {
      ...
      $('#productPriceType').html(formattedProductPriceType);
        ...
        if (tempNormal[1] > 0) {
          $('#regularPrice').html('JulaClub <br><span style="font-size: 42px; line-height: 34px;">' + tempNormal[0] + '<span class="priceSup">' + tempNormal[1] + '</span></span>');
        } else {
          $('#regularPrice').html('JulaClub <br><span style="font-size: 42px; line-height: 53px;">' + tempNormal[0] + '<span style="letter-spacing: -7px; padding-right: 8px;">.-</span>');
        }
    ...
    function adjustElementsBasedOnCampaignText(campaignText) {
      if (!campaignText || campaignText.trim() === "") {
          ...
          // Update styles
          if (productBox) {
              productBox.style.top = "-14px";
              productImage.style.width = "180px";
              productImage.style.height = "160px";
          }
  
          if (productInfo) {
              productInfo.style.top = "-35px";
              priceInfo.style.top = "325px";
              priceInfo.style.right = "-10px";
          }
      }
  }
