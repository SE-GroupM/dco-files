  ...
  if (ccVideo != '') {
    ...
    function updateProductDisplay(index) {
      ...
      if (productPriceType === 'regular') {
        ...
        if (tempNormal[1] > 0) {
          $('#regularPrice').html('JulaClub <br><span style="font-size: 52px; line-height: 34px;">' + tempNormal[0] + '<span class="priceSup">' + tempNormal[1] + '</span></span>');
        } else {
          $('#regularPrice').html('JulaClub <br><span style="font-size: 52px; line-height: 53px;">' + tempNormal[0] + '<span style="letter-spacing: -7px; padding-right: 8px;">.-</span>');
        }
      ...
      } else if (productBadge === 'Julas laveste pris!') {
        $('#productBadge')
          .css({
      ...
      function adjustElementsBasedOnCampaignText(campaignText) {
        if (!campaignText || campaignText.trim() === "") {
            ...
            // Update styles
            if (productBox) {
                productBox.style.top = "-15px";
                productImage.style.width = "200px";
                productImage.style.height = "210px";
            }
    
            if (productInfo) {
                productInfo.style.top = "-45px";
                priceInfo.style.top = "298px";
            }
        }
    }
