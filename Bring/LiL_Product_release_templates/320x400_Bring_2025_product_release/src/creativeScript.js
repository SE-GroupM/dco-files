...
onLemonpiReady(function () {
 ...
 // Set static product image
$("#product_image_wrapper img").attr("src", local_content.dynamic_product_img.value);
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // SEENTHIS variables
    var videoSrc = content.Video_320x400_src.value;
    var videoTracker = content.Video_320x400_tracker.value;
  ...
  if (local_content.Use_default.value == 'TRUE'){
    ...
    const frame1 = $('#frame1');
    const frame2 = $('#frame2');
    const staticVisualsWrapper = $('#static-visuals-wrapper'); // Now contains image and CTA
    ...
    // Initial state for static visuals wrapper (image and CTA)
    gsap.set(staticVisualsWrapper, { opacity: 0, y: 20 }); // Start slightly down and invisible

    var masterTimeline = gsap.timeline({
        repeat: number_of_loop_repeats, // Use Lemonpi value for repeats
        onComplete: function() {
            ...
            // Ensure static visuals (image and CTA) remain visible
            gsap.to(staticVisualsWrapper, { opacity: 1, y: 0, duration: 0 });
            gsap.to(bringLogo, { opacity: 1, y: 0, duration: 0 });
        }
    });

    // Initial reveal of static elements (product image and CTA)
    ...
    function onClick (event) {
      return window.dispatchEvent(
        new CustomEvent('lemonpi.interaction/click', {
          detail: {
            placeholder: 'landingpage_320x400'
          }
      }));
    }
